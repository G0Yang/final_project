...
contractPATH = os.path.join(os.path.dirname(os.path.realpath(__file__)), "ledger")
...
def listup(dir = contractPATH):
    ...
    for i in dirlist:
        if ".json" in i:
            contractList.append(i.split('.json')[0])
    return contractList
...
def findContract(contractID):
    ...
    try:
        data = open(os.path.join(contractPATH, contractID) + ".json").read()
